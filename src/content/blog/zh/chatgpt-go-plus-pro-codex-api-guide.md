...
很多人第一次看 OpenAI 的付费体系，都会卡在同一个问题上：
...
这篇文章只做一件事：把这四个问题讲透，让你在 10 分钟内判断自己该选哪一档。
...
- ChatGPT 订阅解决“我在 ChatGPT 里用 AI”；
- API 解决“我把 AI 接到我自己的系统里”。

## 二、Go、Plus、Pro 到底在卖什么

可以把三档订阅理解成同一条产品线的三个层级：
...
OpenAI 的部分额度会动态调整。下面只列官方页面明确写出的差异，方便你快速比较：
...
先定性：`Codex` 是编程代理（coding agent）。你可以在 CLI、IDE、Web、桌面端里用它做代码理解、修改、运行与协作。
...
还有一个常被忽略但很关键的点：Codex CLI 支持两种认证路径。
...
Go 更像“高配免费版”，重点是把基础能力上限拉高；Plus 则更偏“进阶工具包”，重点在高阶推理、legacy models、deep research、agent 和 Sora。

### 2. Go 能用 Thinking 吗？

能。Go 可以在输入框 `+` 菜单里启用 Thinking，官方公开额度是 `10 条 / 5 小时`。如果你经常做长推理任务，Plus 或 Pro 更稳妥。

### 3. Plus 和 Pro 都能用 Agent 吗？额度差多少？

都能用。当前公开口径是：Plus `40 条 / 月`，Pro `400 条 / 月`。如果你把 Agent 当日常工具，Pro 的差距会非常明显。

### 4. Pro 写着 unlimited，是否等于完全无限？

不等于“无条件无限”。官方明确说明 unlimited 受 abuse guardrails 和使用条款约束；异常高频、自动化滥用或转售场景会触发限制。

### 5. 超过 Go/Plus 的主模型额度后会怎样？

主模型额度在当前窗口内用完后，会自动切到 mini 版本；窗口重置后恢复。这个窗口是滚动计算，不是按自然日清零。
