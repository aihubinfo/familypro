...
- i18n 注册：`src/lib/i18n/registry.ts`
- 页面路由：`src/pages/**`
- SEO 组件：`src/components/SEO.astro`
...
- 不破坏既有路由：`/{lang}/`、`/{lang}/blog/`、`/{lang}/blog/{slug}/`。
...
### 5.3 Hreflang / 多语言
- 多语言页必须输出完整 `hreflang` alternates，并包含 `x-default`。
- `hreflang` 必须与 `LANGUAGE_REGISTRY` 匹配（如 `zh-CN`、`en`、`ja`）。
- `html lang` 与页面语言一致。

### 5.4 Open Graph / Twitter
...
4. 若改了页面渲染/样式：`npm run preview` 并人工抽查关键页面。
5. 若改了 SEO 逻辑：检查页面源码中的 meta/link/json-ld 输出。
6. 输出变更摘要、验证结果、剩余风险。
